...
2. **Evaluate before adopting.** Check [OpenSSF Scorecard](https://scorecard.dev) ratings. Review the project's security policy (`SECURITY.md`). Look at maintenance activity, response to security issues, and governance structure. Five minutes of evaluation can prevent months of problems.
...
4. **Keep dependencies updated.** Enable automated dependency updates through [Dependabot](https://github.com/dependabot), [Renovate](https://github.com/renovatebot/renovate), or similar tools. Review and merge updates promptly. Stale dependencies accumulate vulnerabilities.
...
1. **Deploy comprehensive scanning.** Use tools like [`Snyk`](https://snyk.io), [`Grype`](https://github.com/anchore/grype), [`Trivy`](https://trivy.dev), or similar for vulnerability scanning. Integrate scanning into CI/CD pipelines. Fail builds on critical vulnerabilities.

2. **Implement SBOM processes.** Generate SBOMs using tools like [`Syft`](https://github.com/anchore/syft) or [`cdxgen`](https://github.com/cdxgen/cdxgen). Store and manage SBOMs with platforms like [`Dependency-Track`](https://dependencytrack.org). Use SBOMs for vulnerability correlation and incident response.

3. **Enable provenance verification.** Verify package signatures where available. Use [Sigstore](https://sigstore.dev) for signing your own releases. Check [SLSA](https://slsa.dev) provenance attestations when provided.
...
4. **Fund the ecosystem you depend on.** Your organization relies on open source infrastructure maintained by others. Consider contributing to [OpenSSF](https://openssf.org), sponsoring critical projects, or funding security audits. Ecosystem investment is enlightened self-interest.
...
5. **Pursue Best Practices Badge.** The [OpenSSF Best Practices Badge](https://www.bestpractices.dev) provides a roadmap for security practices appropriate to your project's stage. Even if you don't complete it, the assessment identifies improvements.
...
2. **Seek support for security work.** Security improvements require time and expertise. Explore [GitHub Sponsors](https://github.com/sponsors), [Open Collective](https://opencollective.com), [Tidelift](https://tidelift.com), or foundation affiliation. Apply for audit funding through [OSTIF](https://ostif.org) or similar. Security work deserves compensation.
...
2. **Align with existing frameworks.** Build on [NIST SSDF](https://csrc.nist.gov/projects/ssdf), [OpenSSF](https://openssf.org) guidance, and established standards rather than creating parallel requirements. Consistency reduces compliance burden while increasing actual security.
