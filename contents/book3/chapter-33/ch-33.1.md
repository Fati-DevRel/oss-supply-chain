...
- **Linux kernel**: Rust support merged in 6.1[^linux-rust] (December 2022)
- **NSA/CISA**: Explicit guidance[^nsa-memory-safe] recommending memory-safe languages

CISA's December 2023 report "The Case for Memory Safe Roadmaps"[^cisa-memory-roadmaps] recommends that software manufacturers prioritize memory-safe programming languages to eliminate entire categories of memory safety vulnerabilities from their products.
...
- **Cryptographic implementations**: Verified crypto libraries (HACL*[^hacl-star], Fiat-Crypto)
- **Operating system kernels**: seL4[^sel4] microkernel with complete formal verification
- **Compilers**: CompCert[^compcert] C compiler proved to preserve program semantics
