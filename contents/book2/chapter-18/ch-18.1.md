...
**Maven Central**[^maven-central], accessed through Sonatype's OSS Repository Hosting (**OSSRH**[^ossrh]), requires account verification and namespace (groupId) ownership validation. Publishing requires signing artifacts with PGP keys, which adds a layer of publisher authentication beyond account credentials. The verification process, while more cumbersome than other registries, provides stronger assurance of publisher identity.

**NuGet**[^nuget] uses API keys scoped to specific packages or package prefixes, with support for expiring keys. The NuGet Gallery supports Azure Active Directory integration for organizational accounts, enabling enterprise identity management.
...
The **Eclipse Foundation** announced in January 2026 a commitment to pre-publish security verification for Open VSX, the open-source IDE extension marketplace used by VSCodium, Eclipse Theia, and Gitpod.[^openvsx-prepublish] The announcement was catalyzed by recurring abuse — including the GlassWorm self-propagating worm (October 2025) and a publisher credential compromise affecting four extensions (January 2026) — and explicitly framed Open VSX as "core infrastructure in the developer supply chain."
...
[^ossrh]: Sonatype, "OSSRH Publishing Guide," https://central.sonatype.org/publish/publish-portal-guide/
