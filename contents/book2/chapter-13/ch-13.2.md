...
[SLSA (Supply-chain Levels for Software Artifacts)](https://slsa.dev/) provides a framework for build integrity. SLSA v1.0 organizes requirements into Build Levels:
...
The [Reproducible Builds](https://reproducible-builds.org/) project works toward making all software reproducible:
...
To ensure the integrity of packages downloaded from registries that support signing, npm can verify [registry signatures](https://docs.npmjs.com/verifying-registry-signatures/) via `npm audit signatures`. This requires a sufficiently recent npm version and signed metadata from the registry.
...
[uv](https://docs.astral.sh/uv/) is a fast Python package manager written in Rust that provides strong supply chain security defaults. Unlike traditional `requirements.txt` files where hash verification is opt-in, uv generates lockfiles with SHA-256 hashes by default.
