...
!!! info "Automated Health Checks"
    ...
    OpenSSF Scorecards measures security practices through automated checks: code review requirements, branch protection, vulnerability scanning, dependency updates, fuzzing, SAST, and signed releases.

## OpenSSF Scorecards

**OpenSSF Scorecards**[^openssf-scorecards] is the most comprehensive open source security scoring system, measuring security practices through automated checks against public repositories.
...
| **Vulnerabilities** | Known vulnerabilities in OSV[^osv] database | Critical |
...
| **Fuzzing** | Project is fuzzed (OSS-Fuzz,[^oss-fuzz] etc.) | Medium |
...
**API rate limits:** 5000 requests per hour by default (based on IP address). Full API documentation[^ecosystems-docs] available.
