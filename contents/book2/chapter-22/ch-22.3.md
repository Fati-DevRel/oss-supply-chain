...
1. **Automatic generation**: SBOMs created for every build without team configuration (using tools like [Syft](https://github.com/anchore/syft) or [Trivy](https://trivy.dev/))
2. **Format flexibility**: Platform handles [CycloneDX](https://cyclonedx.org/), [SPDX](https://spdx.dev/), and other formats as needed
...
The platform handles everything else—tool selection, scanning schedules, enrichment data sources, PR generation logic (e.g., using [Dependabot](https://docs.github.com/en/code-security/dependabot) or similar services).
...
| **Runtime protection** | Admission control enforces policies (e.g., [SLSA](https://slsa.dev/) compliance) | Compliant deployments just work |
